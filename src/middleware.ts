...
export async function middleware(request: NextRequest) {
  ...
  // In development mode, if mock_role cookie is set, bypass Stack Auth
  if (isDevelopment && mockRole && ['admin', 'manager', 'user'].includes(mockRole)) {
    // Allow access to dashboard routes in development with mock auth
    if (pathname.startsWith('/dashboard')) {
      return NextResponse.next();
    }
    
    // Redirect to dashboard if accessing root with mock auth
    if (pathname === '/') {
