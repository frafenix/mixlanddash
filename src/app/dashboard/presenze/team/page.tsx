...
export default function PresenzeTeamPage() {
  ...
  useEffect(() => {
    // Check for development mode and mock authentication first
    ...
    if (isDevelopment && typeof window !== 'undefined') {
      ...
      // If mock role is manager or admin, allow access
      if (mockRole && ['manager', 'admin'].includes(mockRole)) {
        return;
      }
    }

    // Fallback to Stack Auth user metadata
    if (user) {
      const metadata = user.clientReadOnlyMetadata as { role?: string };
    ...
    setTimeout(() => {
      const mockRecords: AttendanceRecord[] = [
        ...
        },
        {
          ...
          employeeEmail: "luca.bianchi@company.com",
          teamName: "Sviluppo",
          month: currentMonth,
          totalHours: 155,
          workingDays: 19,
          ...
          status: "submitted",
          submittedAt: new Date(),
          submittedBy: "Luca Bianchi",
        },
        {
          id: "att-3-2025-10",
          employeeId: "3",
          employeeName: "Anna Verdi",
          employeeEmail: "anna.verdi@company.com",
          teamName: "Design",
          month: currentMonth,
          totalHours: 165,
          workingDays: 21,
  ...
  const handleApprove = (id: string) => {
    ...
    if (record && user && record.employeeId === user.id) {
      setNotification({ message: "Non puoi approvare le tue stesse presenze.", type: "danger" });
      return;
    }
    ...
    setNotification({ message: "Presenze approvate con successo!", type: "success" });
  };
  ...
  const handleReject = (id: string, reason: string) => {
    const record = attendanceRecords.find(r => r.id === id);
    if (record && user && record.employeeId === user.id) {
      setNotification({ message: "Non puoi rifiutare le tue stesse presenze.", type: "danger" });
      return;
    }

    setAttendanceRecords(prev =>
      prev.map(record =>
        record.id === id ? { ...record, status: "rejected", rejectionReason: reason } : record
      )
    );
    setNotification({ message: "Presenze rifiutate.", type: "info" });
  };

  // Filtra i record in base ai filtri correnti
