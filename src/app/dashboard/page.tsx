...
export default async function DashboardPage() {
  ...
  // Get user role from server user metadata or mock override
  let userRole = "user";

  // Role override via cookie set by UserSwitcher (dev/testing)
  const cookieStore = await cookies();
  const roleOverride = cookieStore.get('mock_role')?.value;
  if (roleOverride && ['admin','manager','user'].includes(roleOverride)) {
    userRole = roleOverride as 'admin' | 'manager' | 'user';
  } else if ('metadata' in user && user.metadata) {
